...
# This const declaration is non-essential but hey why not
const microphys = MicrophysicsParameterSet(LiquidParameterSet(), IceParameterSet())
...
# ---- MOVING THIS TO TEST IN config_cfsites (line 667...)
...
function atmos_source!(
    ...
    if z_sponge <= z
        ...
        # -- -- moisture then gets adjusted to remain in equilibrium but it is probably undersaturated and is fine... (I hope...)
            ...
            # source.ρe -= β_sponge * (cvm * state.ρ * T_tendency)
        ...
        source.ρe              -= β_sponge * (state.ρe              .-  aux.gcminfo.ρe    ) # relaxe total energy
        source.moisture.ρq_tot -= β_sponge * (state.moisture.ρq_tot .-  aux.gcminfo.ρq_tot) # relaxes moisture maybe?        
        ...
        for reqvar in req_varnames
            if reqvar == varname
                ...
                if varname == "hfls" || varname == "hfss" || varname == "ts" # surface properties, have no lev as dim 1
                    ...
                    # if varname == "ts" # surface properties, have no lev as dim 1
                    #     var = var + 5
                    # end
                else
...
function init_cfsites!(problem, bl, state, aux, localgeo, t, spl)
    ...
    include_vertical_velocity = FT(true)
    ...
    aux.gcminfo.w_s            = w_s * 0  # this is bad for forcing since it can be large in covective states, kill

    # ρ     =     air_density(bl.param_set, aux.gcm.ta, pfull, PhasePartition(hus))
    e_int_gcm = internal_energy(bl.param_set, aux.gcminfo.ta       , PhasePartition(hus))
    e_pot_gcm = _grav * z
    ...
    aux.gcminfo.ρe                = ρ_gcm * e_tot_gcm
    aux.gcminfo.ρ                 = ρ_gcm
...
function config_cfsites( # returns config 
    ...
    problem = AtmosProblem(
        boundarycondition = (
            AtmosBC(
                ...
                energy = PrescribedEnergyFlux((state, aux, t) -> (abs(hfls)*1.5 + abs(hfss)) * test_scale),
                moisture = PrescribedMoistureFlux(
                    (state, aux, t) ->
                        abs(hfls)*test_scale*1.5 / latent_heat_vapor(param_set, ts),
                ),
            ),
            AtmosBC(),
        ),
    ...
    elseif moisture_model == "nonequilibrium"
        ...
        source = (source_default..., CreateClouds(),Rain_1M()) # testing w/ precip?
