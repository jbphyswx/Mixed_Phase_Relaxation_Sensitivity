...
# Alternate implementation that slows liq --> vapor in the presence of supersaturation limited by ice formation. Doesn't create liquid but doesn't let liquid disappear too quick either...
# Different inputs so gets multiple dispatched separately
...
function conv_q_vap_to_q_liq_ice(
    ...
    #print("Calling the alternate incantation... ooooo ")
    ice_dsat = q_sat.ice - q.ice # the delta from saturation for ice...
    liq_dsat = q_sat.liq - q.liq

    if ice_dsat > 0 # we are supersaturated w/ respect to ice, so liq/vap processes (here just evap in principle) are rate limited by ice... 
        # this could blow pu the code when liq_dsat becomces small (imagine we have little water... then the tau goes to 0 and the source being equal to ice gain might fall apart and we oscillate near 0...
        # idk if the code prevents negative values from these sources or just blow up, but in principle we'd only be damped by the convergence of ice... which could lag the driving of liq to 0
        # maybe add a condition here that below some liq threshold with liq_dsat > 0 you just go back to the quick timescale and let it go by itself bounded.
        # at the same time the ice flux *should* be smaller because it has a much smaller timescale so maybe they'll converge together gracefully.... who knows
        # you need ice departures from equilibrium of O(tau_ice/tau_liq) larger than liq to really destabilize if liq is stable... for literature this is O(10^2-4 / 10^(0-1)) ~ O(10^2-5) which is p big so we prolly safe...
        _τ_cond_evap::FT = (-liq_dsat/ice_dsat) .*  τ_relax(ice_param_set)
        _τ_cond_evap = max.(_τ_cond_evap, τ_relax(liquid_param_set)) # pick the slower timescale to ensure stability and rate limiting.... prevents blowups when q-ql small but ice steps big
    else # we are subsaturated w/ respect to ice, so liq/vap processes proceed unimpeeded
        _τ_cond_evap = τ_relax(liquid_param_set)
    end

    return liq_dsat / _τ_cond_evap
end
...
#     ice_dsat = q_sat.ice - q.ice # the delta from saturation for ice...
#     liq_dsat = q_sat.liq - q.liq
#     # print("callaloo: ", ice_dsat, " | ", liq_dsat, " | ", q_sat, " | ", q ,"\n")
#     _τ_cond_evap::FT = ((ice_dsat .> 0) .* max.(-liq_dsat .* τ_relax(ice_param_set) ./ ice_dsat , τ_relax(liquid_param_set)))  .+  ((ice_dsat .<= 0) .* τ_relax(liquid_param_set))
#     # print("alalalalla: ",_τ_cond_evap, "\n")
#     return liq_dsat ./ _τ_cond_evap
# end
