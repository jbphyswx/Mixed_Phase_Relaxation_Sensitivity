...
_replace_me = None
# see http://holoviews.org/user_guide/Plotting_with_Bokeh.html for curve info
def base_data_1d_viewer(   data, 
                            kdims            = None, # key dimensions for the dataset
                            vdims            = None, # variable dimension for the dataset, figure out how 
                            plot_dim         = None, # dimensions for a plot, note for coastlines you must provide 'lon' first to get the correct bounding box
                            plot_vars        = None, # variables  for a plot, FIGURE HOW TO MAKE THIS INTO A SWITCHABLE MENU
                            plot_engine      = holoviews.Curve, # could be geoview.Image, etc depending on the spacing we got (even spacing for example doesn't need QuadMesh). Note, for some reason, geoviews.Quadmesh shits the bed on some projections like ccrs.PlateCarree() or None, no idea why...
                            dynamic          = True,
                            plotter          = None, #holoviews.operation.datashader.datashade, # seems to be fastest and most robust (probably unecessary for 1d)
                            spreader         = None, #holoviews.operation.datashader.dynspread,
                            precompute       = True,
                            line_properties  = {'line_width':100, 'line_color':[0,0,1], 'line_dash':'-', 'line_alpha':0.5},
                            point_properties = {'fill_color':[0, 1, 0], 'fill_alpha':0.5 , 'line_width':5, 'line_color':'black', 'size':500},
                            text_properties  = None, # e.g. {'text_align':'center' , 'text_baseline':'middle', 'text_color':'gray', 'text_font':'Arial'},
                            framewise        = False,
                            show_grid        = False,
                            fig_size         = (300,300),
                            interactive      = True,
    ...
    plot_lines  = (line_properties  is not None)
    plot_points = (point_properties is not None)
    plot_text   = (text_properties  is not None)
...
#             print_verbose('...failed, continuing with no projection')
    ...
    if plot_dim is None: plot_dim=kdims[0] # default to plotting over first first dimension
#     if is_geospatial: # must be of form ['lon','lat'] for proper plotting
#         if ('lat' in plot_dims[0].lower()) and ('lon' in plot_dims[1].lower()):
#             print_verbose('flipping [lat,lon] to [lon,lat] for geospatial plotting')
#             plot_dims = [plot_dims[1],plot_dims[0]] # opposite orientation is fine, otherwise just will fail in plotting probably, who knows
    ...
    if plot_vars is None: plot_vars=vdims
    def plot_engine_output(engine, xr_dataset=xr_dataset, kdims=plot_dim, vdims=plot_vars, dynamic=dynamic,engine_opts=None):
        print(vdims,plot_vars, xr_dataset,end="\n\n")
        if engine_opts is None: engine_opts = {}
        print(engine,engine_opts)
        return xr_dataset.to(engine, kdims=plot_dim, vdims=plot_vars, dynamic=dynamic).opts(**engine_opts)
#     plot_engine_output = xr_dataset.to(plot_engine, kdims=plot_dim, vdims=plot_vars, dynamic=dynamic)
...
#             return out
    ...
    if plot_lines:
        plot_engine = holoviews.Curve
        properties  = line_properties
        out = plot_engine_output(plot_engine, engine_opts=properties)
    print(' first: ' , out.opts.info())

        
    if plot_points:
        plot_engine = holoviews.Scatter
        properties  = None

        if out is None:
            out = plot_engine_output(plot_engine, engine_opts=properties)
        else:
            out = out * plot_engine_output(plot_engine, engine_opts=properties)
    print(' second: ' , out.opts.info())
    print(out, 'yuhhhh')
    
    if plot_text: # not sure this will work haha
        plot_engine = holoviews.Text
    ...
    print(' finally: ' , out.opts.info())
    out = out.opts(holoviews.opts.Curve(**line_properties))
    print(' finally2: ' , out.opts.info())
    out = out.opts(holoviews.opts.Scatter(**point_properties))
    print(' finally3: ' , out.opts.info())

    #, holoviews.opts.Text(**text_properties))
...
#     import geoviews.feature as gf
    ...
    dims_here =  [x for x in kdims if x != plot_dim]
    out = holoviews.DynamicMap(out, kdims=dims_here)
#     out = out.select(lev=1)
    redims = {}
    for dim in dims_here:
        redims[dim] = tuple(data[dim].values)
    print(redims)
#     kdim_maps = dict(zip(kdims[::-1], np.arange(len(kdims))))
    out = out.redim.values(**redims)

#     out = spreader(plotter(out))
    if plotter is not None:
        out =  plotter(out,precompute=precompute)
#     print(type(out), plotter)
#     print('out yeah: ', out)
    if spreader is not None:
        out = spreader(out)
        
    out = out.options(show_grid=show_grid,aspect=aspect, width=fig_size[0], height=fig_size[1],tools=['xwheel_zoom','ywheel_zoom', 'xpan','ypan','hover'], active_tools=['xwheel_zoom', 'pan'])
#     from holoviews.streams import Pipe, Buffer
    
#     variables= list(data.data_vars)
#     pipe = Pipe(data=variables[0])
#     plot_engine_output = holoviews.DynamicMap(plot_engine_output,streams=[pipe])
#     out = panel.Row(panel.panel(explorer.param, parameters=['variable']), out )#plotter(plot_engine_output,precompute=precompute).options(cmap=cmap, colorbar=colorbar, clim=clim, show_grid=show_grid,aspect=aspect))
#     if xlabels is not None:
#         print_verbose('relabeling x axis values')
#         out = out.relabel().opts(xticks=[*zip(xticks,xlabels)])
# #         out = out.relabel('Tick formatters').opts(xformatter=lambda x: xlabel[x])
...
def data_1d_viewer(data, **kwargs):
    """ kwargs last updated from base_data_map_viewer above on 09/29/2019, if an error in this list check for updated kwargs in base_data_map_viewer
    kdims        = None, # key dimensions for the dataset
    vdims        = None, # variable dimension for the dataset, figure out how 
    plot_dims    = None, # dimensions for a plot, note for coastlines you must provide 'lon' first to get the correct bounding box
    ...
    class sel_dset_var(): # inherit kwargs from data_map_viewer
        def varget(dset,var,**kwargs):
            in_args = kwargs
            for arg,argval in in_args.items():
                if arg not in ['line_prperties','point_properties','text_properties']:
                    if isinstance(argval,dict): in_args[arg] = argval[dset] #convert dict listings to str8 up if we provided a dict
            out = base_data_1d_viewer(data[dset][var], **kwargs)
            return  out# use above fcn...
